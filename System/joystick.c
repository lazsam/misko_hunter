...
void JOY_init(ADC_HandleTypeDef *ADC_handle, TIM_HandleTypeDef *timer_handle)
{
	...
	// 2. Nastavitev začetnih vrednosti "joystick" sistemskih spremenljivk
		...
		joystick.position_raw_min[X] = 856;

		joystick.position_raw_min[Y] = 1095;


		joystick.position_raw_max[X] = 3240;

		joystick.position_raw_max[Y] = 3338;
...
uint8_t JOY_get_axis_position(joystick_axes_enum_t axis)
{
	...
	relative_position = (joystick.position_raw[axis] - joystick.position_raw_min[axis]) * 101 / (joystick.position_raw_range[axis] + 1);
...
void JOY_button_demo(void)
{
	...
	LED_set(LED1, !JOY_get_button_state() );
...
void JOY_LED_demo(void)
{
    ...
    uint8_t position = JOY_get_axis_position(X);
    if (position < 15)
        LEDs_write(0x80);
    else if (position < 25)
        LEDs_write(0x40);
