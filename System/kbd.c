...
typedef struct
{
    ...
    button_sig_value_t state_old; // informacija o prejšnjem stanju tipke
    button_sig_value_t state_new; // informacija o trenutnem stanju tipke
...
void KBD_init(void)
{
	...
	// 1. Inicializirajte "handle" strukture za posamezne tipke v tipkovnici na strojnem nivoju
		...
		// buttons on port C
		keyboard.buttons[ BTN_OK ].pin  = 15;
		keyboard.buttons[ BTN_OK ].port = GPIOC;

        keyboard.buttons[ BTN_ESC ].pin  = 14;
        keyboard.buttons[ BTN_ESC ].port = GPIOC;

		// buttons on port G
        keyboard.buttons[ BTN_UP ].pin  = 0;
        keyboard.buttons[ BTN_UP ].port = GPIOG;

        keyboard.buttons[ BTN_DOWN ].pin  = 1;
        keyboard.buttons[ BTN_DOWN ].port = GPIOG;

        keyboard.buttons[ BTN_RIGHT ].pin  = 8;
        keyboard.buttons[ BTN_RIGHT ].port = GPIOG;

        keyboard.buttons[ BTN_LEFT ].pin  = 6;
...
void KBD_scan(void)
{
	...
	for(int i=0; i < NUM_OF_BTNS; i++)
	{
		...
		keyboard.buttons[i].state_new = (LL_GPIO_ReadInputPort(keyboard.buttons[i].port) >> keyboard.buttons[i].pin) & 0x01;
		...
		if ( keyboard.buttons[i].state_old == BTN_SIG_VALUE_RELEASED && keyboard.buttons[i].state_new == BTN_SIG_VALUE_PRESSED)
		{
